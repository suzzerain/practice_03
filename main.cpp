...
void spiralFill(int *start, int N){
    ...
    for (;countPrinted != N * N;) {
        ...
        for (int *first = (start + countPrinted), *last = first + (N - 1), i = countGlobalIterations * (countGlobalIterations > 0? 3:0); i < ((N * 3) - (3* countGlobalIterations)) &&  first <= last; i += 3, first++) {
        ...
        for (int *first = (start + countPrinted), *last = first + (N - 1), i = countGlobalIterations + 1; i < (N - countGlobalIterations) &&  first < last; first++, i++) {
        ...
        for (int *first = (start + countPrinted), *last = first + (N - 1), i = N - 2 - countGlobalIterations; i >= (1 + countGlobalIterations) && first <= last; i--, first++) {
...
//    system("cls");
    ...
    int count = 0;
    for (int k = 1; k<=N/2; k++) {
        for (int *first = start + count, *end = first + N-1, x = 0, y = 0; first <= end; first++, x++, y++) {
